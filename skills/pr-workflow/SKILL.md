...
이 스킬이 **PR 생성 단계의 단일 기준(SSOT)** 이다. `pr-completion-loop`(생성 후 완주 루프)를 포함해 다른 스킬이 PR을 생성할 때도 반드시 이 절차를 따르며, 여기의 규칙(프리뷰 댓글 형식·검증, PR 본문 템플릿, 스크린샷 섹션)을 다른 문서에 복제하지 않는다.

## 사용 시점

-   사용자가 PR 생성 또는 PR 준비를 요청했을 때
-   코드 정리/컨벤션 점검 후 PR까지 한 번에 처리해야 할 때
-   `pr-completion-loop` 등 다른 스킬이 PR 생성 단계에 도달했을 때 (선행 스킬로 호출됨)
...
7. 프리뷰 배포 댓글 작성
    ...
    - **댓글 본문은 반드시 슬래시로 시작하는 `/preview:<앱>` 한 줄이다. 슬래시 없는 `preview:cue`는 오류 없이 조용히 무시되어 프리뷰가 생성되지 않는다.** 등록 직후 댓글 본문만 확인한다:
        ```bash
        gh pr view <pr-number> --json comments --jq '.comments[-1].body'   # 정확히 "/preview:<앱>" 인지
        ```
        run 생성·배포 완료는 **기다리거나 폴링하지 않는다** — 프리뷰는 리뷰어용 절차이고 검증 근거는 로컬 dev 서버다. run 생성 여부는 작업 마무리 시점에 한 번만 조회해 기록한다:
        ```bash
        gh run list --workflow "[PREVIEW]UNIFIED" --limit 3 --json status,displayTitle,createdAt
        ```
        그때 run이 없고 댓글에 슬래시가 빠져 있으면 삭제 후 재등록한다.
...
-   **UI 파일(`.svelte`, `.tsx`, `.css` 등) 변경이 diff에 있으면 "### 스크린샷" 섹션을 빈 주석으로 남기지 않는다.** 캡처 가능한 상태면 스크린샷을 첨부하고(GitHub `user-attachments` URL, 웹 세션이 없으면 `ytw418/pr-assets` raw URL fallback — `pr-completion-loop` 스킬 4단계 참고), 즉시 캡처가 불가하면 `스크린샷: 브라우저 검증 후 첨부 예정`을 명시해 첨부가 미완료 상태임을 PR에 드러낸다. 스크린샷 이미지 파일을 저장소에 커밋하지 않는다.

### 기존 PR 업데이트 시 프리뷰 갱신

-   이미 생성된 PR에 커밋을 추가로 푸시한 경우, 작업 마무리 시점에 한 번 기존 프리뷰 트리거 댓글을 현재 사용자 작성분에 한해 삭제한 뒤 새 `/preview:cue` 댓글을 등록한다. push마다 반복하지 않고 배포 완료도 기다리지 않는다.
