...
3. 커밋·push 후 Draft PR을 만들고 변경 앱의 preview 댓글만 등록한다. 댓글 본문은 **슬래시로 시작하는 정확히 한 줄** `/preview:<앱>` (예: `/preview:cue`)이어야 `[PREVIEW]UNIFIED` 워크플로우가 트리거된다. **슬래시를 빼면(`preview:cue`) 아무 오류 없이 조용히 무시된다.** 등록 후 반드시 검증한다:

   ```bash
   # 방금 단 댓글 본문이 정확히 "/preview:<앱>"인지 확인
   gh pr view <PR 번호> --json comments --jq '.comments[-1].body'
   # 1~2분 내 preview 워크플로우 run이 생성됐는지 확인. 없으면 댓글을 삭제하고 슬래시 포함으로 재등록한다.
...
8. 앱 코드 커밋을 push했거나 작업 완료·검증 결과 코멘트를 남길 때는 변경 앱의 프리뷰 갱신 코멘트(`/preview:<앱>` — 슬래시 필수)를 다시 등록하고 3항과 같은 방법으로 트리거 여부를 검증해, 리뷰어가 최신 head 기준 프리뷰를 보게 한다. 문서·스킬만 바뀐 push에는 생략한다.
