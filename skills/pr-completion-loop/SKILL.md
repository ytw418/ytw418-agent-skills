...
3. 커밋·push 후 Draft PR 생성과 변경 앱의 preview 댓글 등록·트리거 검증은 `pr-workflow` 7단계 절차를 그대로 따른다. 핵심만 요약하면: 본문은 슬래시로 시작하는 `/preview:<앱>` 한 줄이어야 하고(슬래시 없으면 조용히 무시됨), 등록 후 워크플로우 run 생성까지 확인해야 등록 완료다.
...
8. 앱 코드 커밋을 push했거나 작업 완료·검증 결과 코멘트를 남길 때는 `pr-workflow`의 프리뷰 갱신 절차(기존 트리거 댓글 정리 후 `/preview:<앱>` 재등록, 트리거 검증 포함)를 따라 리뷰어가 최신 head 기준 프리뷰를 보게 한다. 문서·스킬만 바뀐 push에는 생략한다.
