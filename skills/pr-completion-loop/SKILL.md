...
이 스킬은 **PR 생성 이후의 완주 루프**만 담당한다. PR 생성 자체(사전 점검, Draft 생성, PR 본문 템플릿, 프리뷰 댓글 등록·검증)는 **`pr-workflow`가 단일 기준(SSOT)** 이다. 루프를 시작하기 전에 반드시 `pr-workflow`를 먼저 읽고 그 절차대로 PR을 생성한다. 그 절차를 이 문서에 복제하거나 다르게 수행하지 않는다 — 두 문서가 어긋나면 `pr-workflow`가 우선한다.

상황에 맞게 다음 스킬을 읽고 적용한다.

- PR 생성·프리뷰 댓글(SSOT): `pr-workflow`
...
### 0.1 시간 절약 규칙 (모든 모드 공통)

루프가 길어지는 원인은 대부분 "외부 시스템 대기"다. 아래는 기다리지 않는다.

- **프리뷰 배포를 기다리지 않는다.** `/preview:<앱>` 댓글은 리뷰어용 절차이며 검증 근거가 아니다. 등록 직후 댓글 본문이 슬래시로 시작하는지만 확인하고, 워크플로우 run 생성·배포 완료·"반영 커밋" 댓글은 폴링하지 않는다. run 생성 여부는 최종 handoff 직전에 딱 한 번 조회해 기록만 한다.
- **판정은 항상 로컬이다.** 브라우저 검증·스크린샷은 워크트리의 로컬 dev 서버(+Playwright/CDP)에서 만든다. 프리뷰 링크로 검증하지 않는다.
- **봇 리뷰(CodeRabbit·Gemini 등)를 기다리지 않는다.** 사용량 제한(rate limit) 안내가 뜨면 재요청(`@coderabbitai review`)하거나 해제를 기다리지 않는다. handoff 시점에 이미 올라온 코멘트만 한 번에 판정하고, 이후 도착분은 사용자에게 남긴다. 봇 check가 PENDING이면 최대 한 번(60초 이내)만 재조회한다.
- **렌더링이 바뀌지 않은 커밋은 스크린샷을 다시 찍지 않는다.** 폴백 제거·리팩토링·주석·테스트처럼 화면 출력이 동일한 변경이면 직전 캡처를 그대로 쓰고 handoff에 "캡처 SHA ≠ head SHA, 렌더링 동일" 근거를 한 줄 남긴다. UI 출력이 실제로 바뀐 경우에만 재캡처·재업로드한다.
- **dev 서버는 handoff까지 유지한다.** 캡처마다 서버를 내렸다 올리지 않는다. 종료는 마지막에 한 번.
- **대기는 백그라운드로, 작업은 병렬로.** 외부 대기(생성 Action, 폴링)는 background로 걸고 그 사이 다른 게이트를 진행한다. 동일 SHA에 대한 같은 조회를 반복하지 않는다.
...
3. 커밋·push 후 Draft PR 생성과 변경 앱의 preview 댓글 등록은 `pr-workflow` 7단계 절차를 그대로 따른다. 핵심만 요약하면: 본문은 슬래시로 시작하는 `/preview:<앱>` 한 줄이어야 하고(슬래시 없으면 조용히 무시됨), 등록 직후 댓글 본문만 확인한다. run 생성·배포 완료는 기다리지 않는다(0.1 참고).
4. 원격 head SHA를 다시 조회한다. 이후 모든 검증 결과에 SHA를 연결한다.
5. 정적 검사, 관련 단위 테스트, 앱 빌드 또는 타입 검사를 실행한다. baseline 실패와 변경으로 생긴 실패를 분리한다.
6. UI 변경이면 워크트리 로컬 dev 서버의 인증된 route에서 상호작용을 검증한다. 격리된 Playwright(필요 시 CDP)로 판정하고, 같은 캡처를 최종 스크린샷으로 쓴다. 실제 Chrome 재확인은 사용자가 요청할 때만 한다.

## 4. 브라우저·Figma 판정

1. 정확한 Figma node, route, viewport, data state를 고정한다.
2. 비교는 로컬 dev 서버 + 격리된 Playwright로 수행한다. 프리뷰 배포 페이지는 비교 대상이 아니다. 실제 Chrome 재확인은 사용자가 요청할 때만 한다.
...
8. 프리뷰 갱신(기존 트리거 댓글 정리 후 `/preview:<앱>` 재등록)은 push마다 하지 않고 **최종 handoff 직전에 한 번**만 `pr-workflow` 절차로 수행한다. 등록 후 배포를 기다리지 않는다. 문서·스킬만 바뀐 push에는 생략한다.

문서·PR 본문·스킬만 바뀐 커밋 때문에 앱 브라우저 검증을 반복하지 않는다. 화면 출력이 실제로 바뀐 코드가 마지막 캡처 뒤에 들어왔을 때만 재캡처한다(0.1 참고).
...
10. 필수 CI checks가 진행 중이면 상태 변화가 예상되는 간격으로 poll하고 60초 이내에 사용자에게 상태를 갱신한다. 봇 리뷰 check(CodeRabbit 등)는 필수 CI가 아니므로 기다리지 않는다(0.1 참고). 동일 SHA의 동일 결과를 반복 출력하지 않는다.
...
- 마지막 **화면 출력이 바뀐** 코드 변경 후 동일 조건의 final screenshot이 있다. 이후 커밋이 렌더링을 바꾸지 않았으면 직전 캡처를 재사용하고 그 근거를 handoff에 남긴다.
...
**완료·차단 어느 경우든 handoff 직후 반드시 `PushNotification`으로 사용자에게 알린다** (PR URL·상태 한 줄). 사용자는 루프가 도는 동안 자리를 비우므로, 알림 없이 텍스트 보고만 남기는 것은 미완료다.

다음을 보고하고 feature PR은 열린 상태로 둔다.

- feature PR과 자동 merge한 생성 PR URL
- 최종 head SHA (캡처 SHA가 다르면 "렌더링 동일" 근거 한 줄)
- 프리뷰 댓글 등록 여부와 run 생성 여부(1회 조회 결과, 배포 완료는 보고 대상 아님)
- 실행한 검증과 리뷰 대응
