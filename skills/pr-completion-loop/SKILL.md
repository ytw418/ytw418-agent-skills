...
10. checks가 진행 중이면 약 30초 간격으로 poll하고 60초 이내에 사용자에게 상태를 갱신한다.
...
- `deadline` 장기 실행이나 직전에 상태가 변한 CI·리뷰가 있으면 같은 SHA에서 30초 이상 간격의 두 번의 조회가 모두 clean이다. 일반 `fast`·`standard`는 마지막 단일 clean 조회로 충분하다.
